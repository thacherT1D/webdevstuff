...
Inside the `render()` method of a `<Track>` component, its `track` props are combined with its presentation logic to produce a user interface. The user interface allows a user to view and update the track's information.  When the `<button />` component is clicked, the `handleClick()` method is invoked.
