...
In the example above, the `ReactDOM.render()` method is instructed to mount an `<App />` component by creating a component hierarchy and inserting its HTML elements into the DOM hierarchy. The mounting process starts by instantiating a new `<App />` component and passing in a `{ greeting: 'Hello' }` props object. Then, the component's `getInitialState()` method is invoked which initializes a `{ who: 'world' }` state object.

Next, React invokes the component's `render()` method. The `render()` combines the `this.props` and `this.state` objects with its presentation logic and returns a component hierarchy. React uses the component hierarchy to generate and insert HTML elements into the DOM hierarchy. Once mounting is complete, React holds onto the component hierarchy for later.
