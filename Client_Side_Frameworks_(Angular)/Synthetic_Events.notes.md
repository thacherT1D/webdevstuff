...
Now that you've gotten some practice with React's tools, let's build a sophisticated React application. For that, you'll need to learn about React's synthetic event system. A **synthetic event** is an object that wraps a browser's native event object.
