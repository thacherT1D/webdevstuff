...
Form components such as `<input />`, `<textarea />`, and `<select>` differ from other HTML components because their state can be changed by a user. However, when a form component's state is set by a React application, it's known as a **controlled component**.
