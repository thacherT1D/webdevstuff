...
There are many, many build tools for JavaScript; Brunch, Grunt, Gulp, Brocolli, Webpack just to name a few. The reason why there are so many build tools is because they all excel at different things. For now, we're going to use Brunch because it excels at being easy of use out-of-the-box. However, you'll certainly encounter other build tools in the wild.
