...
**NOTE:** When working with a date, a good habit to form is to always include a time with a [timezone](https://en.wikipedia.org/wiki/Time_zone#/media/File:Standard_World_Time_Zones.png). For example, if it's March 31st for a person in the U.S.A., it's April 1st for a person in China.
...
**NOTE:** When an error is generated by a PostgreSQL server that's managing this cluster, the error message is logged to the `server.log` file.

There are a bunch of ways to start a PostgreSQL server that'll manage this cluster. One of the easiest ways is to launch a PostgreSQL server is as a service. A **service** is any server application that the operating system launches as a background process on boot and is restarted by the operating system if it crashes. In other words, you just start the service once and the operating system takes care of keeping it running. As you read this, there are lots of services running on your machine which you can see by opening the `Activity Monitor` application with Spotlight.

![](https://i.imgur.com/3UaQn1Q.png)

**NOTE:** A service can be launched by both the `root` user and your own user account.
