...
A **service** is any server application that the operating system launches as a background process when it boots up. Once launched, the operating system will restart the service if it crashes. In other words, you start the service once and the operating system will keep it running indefinitely. The only way a service stops running is if you command the operating system to do so. As you can imagine, this is a very popular strategy for running a server in a production environment.
...
**NOTE:** A service can be launched by both the `root` user account and your own user account.
