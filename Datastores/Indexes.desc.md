...
For example, imagine a `movies` table with an `id` column as its primary key. Because it's a primary key, PostgreSQL automatically creates an `movies_pkey` index for the table-column pair when the table is created. Over time, thousands of rows are inserted into the table. Eventually, PostgreSQL receives a query to select the row from the `movies` table where its `id` is `1001`. Instead of sequentially searching the entire `movies` table, PostgreSQL can use the `movies_pkey` index to locate the desired row in fraction of the time.
