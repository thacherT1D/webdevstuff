...
- Use the `fs` and `path` modules to manage the file system.
...
## How do you use the `fs` and `path` modules to manage the file system?

The `fs` module is the built-in Node.js API for managing a computer's filesystem. As you can see from the [documentation][fs], the `fs` module can perform a plethora of I/O operations. These operations have both an asynchronous and synchronous method (e.g. `readFile` and `readFileSync`).

The asynchronous method always takes a callback as its last argument that's called when the I/O operation completes. The callback's first argument is always reserved for an exception that's typically generated because of an operating system error. The callback many have other arguments, but it varies from method to method. When an I/O operation completes successfully, the callback's first argument will be `null` or `undefined`. When using the synchronous method, on the other hand, any exceptions that are generated by the I/O operation are immediately thrown.

The `path` module is the built-in Node.js API for handling and transforming file paths across different operating systems. This is necessary to write operating system agnostic applications as the path delimiters on Unix-based operating systems is `/` while on Windows it's `\`. As you can see from the [documentation][path], the `path` module is just a collection of utilities that don't perform any I/O operations. In other words, the methods in the `path` module don't consult the filesystem to see whether or not a path is valid.

Now, it's time to play around with the `fs` and `path` modules. Imagine you're throwing a party and you need to keep a database of all the guests you plan on inviting. Instead of maintaining a Google Spreadsheet, you decide to build a small, command-line application, written in Node.js, that'll accept a few subcommands and log the results to the console. The subcommands will manage the content of a database, which will be a JSON-formatted `guests.json` file.
