...
Assuming authentication is successful, the server starts the authorization process by sending an authentication token in its response.
...
ETag: W/"70-1qLFVreC078yebGK0TQomg"
Set-Cookie: token=eyJ1c2VySWQiOjF9.eyJ1c2VySWQiOjF9nsfSDGJHdfhsiudfhsdfisdh.weguiewhguiwghreuewwgfw; path=/; httponly
...
As you can see, two cookies are sent from the server to the client using the `Set-Cookie` header. You'll see how the token is generated by the server and how a token is secured.
...
```shell
http -v GET localhost:8000/playlists 'Cookie:token=eyJ1c2VySWQiOjF9.eyJ1c2VySWQiOjF9nsfSDGJHdfhsiudfhsdfisdh.weguiewhguiwghreuewwgfw;'
```
...
Connection: keep-alive
Cookie: token=eyJ1c2VySWQiOjF9.eyJ1c2VySWQiOjF9nsfSDGJHdfhsiudfhsdfisdh.weguiewhguiwghreuewwgfw;
Host: localhost:8000
...
│   {                 │            │  {                  │              │                     │
│     alg: 'HS256',   │            │    "alg": "HS256",  │              │                     │
│     typ: 'JWT'      │─── JSON ──▶│    "typ": "JWT"     │─── base64 ──▶│  eyJ1c2VySWQiOjF9   │
│   }                 │  stringify │  }                  │    encode    │                     │
│                     │            │                     │              │                     │
│                     │            │                     │              │                     │
└─────────────────────┘            └─────────────────────┘              └─────────────────────┘
