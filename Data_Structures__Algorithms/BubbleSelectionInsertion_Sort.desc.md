...
Complexity - Although it is one of the elementary sorting algorithms with O(n^2) worst-case time, use insertion sort when the data is nearly sorted (because it is adaptive) or when the problem size is small (because it has low overhead).
...
- In [Java](https://github.com/gSchool/computer-science-exercises/blob/master/src/main/java/BasicSorting.java)
